...
- `!system reload` - Reload bancho settings (the one that are editable from AAP)  
...
- `!mirai reconnect` - Reconnect FokaBot if he's not on online users list anymore  
...
- `!changeusername <target> <username>` - Change a user's username in-game  
